...
export async function analizarfotoHorometro(
    ...
    for(let intento = 1; intento <= intentos; intento++){
        try{
            ...
            console.warn(`⚠️Intento ${intento} fallido al consultar la IA Gemini: ${error.message} || error`);
            ...
            //Pausa de 2.5 segundos antes de realizar el segundo intento
            await new Promise((resolve) => setTimeout(resolve, 2500));
        }
    }
    throw new Error('Error al procesar la imagen con la Inteligencia Articial');
